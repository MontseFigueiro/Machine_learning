...
Repite k-veces la validaci髇 cruzada. Para especificar el tipo de remuestreo:
##La funcion trainControl:
El trainControl genera par醡etros que controlan m醩 a fondo c髆o se crean modelos, con posibles valores:
* method : El m閠odo de remuestreo : "boot", "cv", "LOOCV", "LGOCV", "repeatedcv", "timeslice", "none" and "oob" . El 鷏timo valor , fuera de la bolsa de estimaciones , s髄o puede ser utilizado por los random forest, bagged trees, bagged earth, bagged flexible discriminant analysis, or conditional tree forest models . GBM modelos no est醤 incluidos ( el mantenedor del paquete GBM ha indicado que no ser韆 una buena idea elegir valores de los par醡etros de ajuste en base a las estimaciones de error modelo fuera de banda con 醨boles Mejorado) . Tambi閚, para dejar uno de las cruzadas a cabo la validaci髇, no hay estimaciones de incertidumbre se refieren a las medidas de rendimiento resampled.
* Number and repeats: number controla con el n鷐ero de pliegues en K - veces validaci髇 cruzada o el n鷐ero de iteraciones de remuestreo. repeats hace repetir K - veces la validaci髇 cruzada . Supongamos que method = " repeatedcv " , number = 10 y repeats = 3 , a continuaci髇, tres tiradas de 10 veces las validaciones cruzadas se utilizan como el esquema de remuestreo .
...
train.control
```
Los dos primeros argumentos para entrenar son los objetos de predicci髇 y 
el resultado de datos , respectivamente. El tercer argumento , el m閠odo , 
especifica el tipo de modelo. 
Para ilustrar esto, vamos a ajustar un modelo de 醨bol impulsado a trav閟 del paquete de GBM . 
La sintaxis b醩ica para el montaje de este modelo usando 
repetida de validaci髇 cruzada es:
```{r}
gbm1 <- train(Default ~ ., data = training,
                 method = "gbm",
                 trControl = train.control,
                 ## Esta es una opcion que pasa a trav閟 de gbm
                 verbose = FALSE)
gbm1
```
```{r}
gbm2 <- train(Default ~ ., data = training,
              method = "gbm",
              trControl = train.control,
              ## Esta es una opcion que pasa a trav閟 de gbm
              verbose = FALSE)
...
gbm2
```
Para un modelo GBM tenemos tres par醡etros de ajuste:
  

* n鷐ero de iteraciones, es decir, 醨boles, (llamados n.trees en la funci髇 de GBM)
* complejidad del 醨bol, llamado interaction.depth
* tasa de aprendizaje : la rapidez con que se adapta el algoritmo , llamado shrinkage
* el n鷐ero m韓imo de muestras del conjunto de entrenamiento en un nodo para iniciar la divisi髇 ( n.minobsinnode )

#interpretacion

"train" puede pre - procesar los datos de varias maneras antes de 
ajustar el modelo . La funci髇 "preProcess" se utiliza de forma autom醫ica. Esta funci髇 se puede utilizar
para el centrado y la escala, la imputaci髇 ( ver detalles m醩 abajo ), la aplicaci髇 de la transformaci髇 
signo y extracci髇 de caracter韘ticas espaciales a trav閟 de an醠isis de componentes principales o an醠isis de 
componentes independientes.
La funci髇 train tiene un argumento llamado preProcess toma un character string, para imputaci髇 hay 3 m閠odos:

* k-vecinos:toma una muestra con los valores perdidos y encuentra el k muestras m醩 cercanas en el conjunto 
...
los predictores utilizados en el c醠culo son los que tienen valores perdidos para esa muestra y no hay valores 
que faltan en el conjunto de entrenamiento.

* Otro m閠odo consiste en ajustar un modelo de 醨bol en bolsas para cada predictor usando las muestras del 
conjunto de entrenamiento . Esto suele ser un modelo bastante preciso y puede manejar los valores perdidos .
Cuando un predictor para una muestra requiere la imputaci髇, los valores de los otros predictores son alimentados
a trav閟 del 醨bol bolsa y la predicci髇 se usa como el nuevo valor. Este modelo puede tener coste computacional 
significativa .
...
El tuneGrid argumento puede tener una trama de datos con columnas para cada par醡etro de ajuste . Los nombres de 
columna deben ser los mismos que los argumentos de la funci髇 de ajuste . Para el ejemplo de RDA se ha mencionado 
anteriormente , los nombres ser韆n gamma y lambda. tren sintonizar el modelo sobre cada combinaci髇 de valores de 
las filas .
```{r}
...
model.gbm
```
Para boosted tree model, podemos fijar la tasa de aprendizaje y evaluar m醩 de tres valores de "n.trees":
...
La funci髇 de gr醘icos se puede utilizar para examinar la relaci髇 entre las estimaciones de rendimiento
y los par醡etros de ajuste . Por ejemplo , un simple invocaci髇 de la funci髇 muestra los resultados de la 
primera medida de la ejecuci髇 :
...
```
##summarize results
no hay preproceso, tama駉 conjuntos de entrenamiento, 1372-1235 obs.
